...
- **英文短信编辑**：
  ...
  - 发送后 FPGA 内部短信缓存不清空，串口屏 `t0` 文本框内容也不需要清空。
  - Clear 按钮建议先执行 `t0.txt=""`，再向 FPGA 发送单字节 `0x18`，用于停止重复发送并清空 FPGA 内部缓存。
- **编码格式**：
  - 16 字节前导码 `0x55`，用于接收端建立位/字节同步。
  - 1 字节起始符/同步字节 `0x7e`，接收端以它作为一帧数据的正式开始。
  - 1 字节长度。
  - ASCII 载荷，每字节最低位先发。
  - 1 字节 XOR 校验。
  - 1 字节结束符 `0x7f`，接收端收到校验后再检查该字节，作为本帧结束确认。
